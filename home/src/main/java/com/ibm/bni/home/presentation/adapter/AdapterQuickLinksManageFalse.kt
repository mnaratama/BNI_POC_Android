...
class AdapterQuickLinksManageFalse(
    val context: Context,
    val onItemClickCallback2: ClickCallback,
) : RecyclerView.Adapter<AdapterQuickLinksManageFalse.ViewHolder>() {
    ...
    fun setListItem(itemList: ArrayList<QuickLinksModel>) {
        val contentsQuickLinksManageFalse = arrayListOf<QuickLinksModel>()
        itemList.forEach {
            if (!it.status) {
                contentsQuickLinksManageFalse.add(it)
            }
        }
        this.list = contentsQuickLinksManageFalse
